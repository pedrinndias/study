document.addEventListener('DOMContentLoaded', function() {
    ...
    window.toggleFavorito = async function(equipeId) {
        ...
        try {
            ...
            const iconDetail = document.getElementById(`fav-icon-detail-${equipeId}`);
    ...
    async function main() {
        try {
            ...
            if (!equipesResponse.ok) {
                throw new Error(`Erro ao carregar equipes: ${equipesResponse.status}`);
            }
            ...
            if (!noticiasResponse.ok) {
                throw new Error(`Erro ao carregar notícias: ${noticiasResponse.status}`);
            }
            ...
            const newsContainer = document.getElementById('news-cards-scroller');
            const equipesContainer = document.getElementById('equipes-container');

            const errorMessage = `
                <div class="bg-red-900/50 border border-red-500 rounded-lg p-6 text-center">
                    <i class="fas fa-exclamation-triangle text-red-500 text-3xl mb-3"></i>
                    <h3 class="text-white text-lg font-bold mb-2">Erro ao Conectar com a API</h3>
                    <p class="text-gray-300 mb-4">Verifique se o JSON Server está rodando na porta 3000</p>
                    <code class="bg-gray-900 px-3 py-1 rounded text-sm text-gray-400">npm start</code>
                    <p class="text-gray-400 text-xs mt-3">${error.message}</p>
                </div>
            `;

            if (newsContainer) newsContainer.innerHTML = errorMessage;
            if (equipesContainer) equipesContainer.innerHTML = errorMessage;
        }
    }
